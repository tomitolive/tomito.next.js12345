...
def download_tmdb_image(tmdb_poster_path):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            log.info(f"Mirrored asset: {filename}")
            return filename
    except Exception as e:
        log.error(f"Failed to mirror asset {filename}: {e}")
    return None
...
def create_page(item_data, media_type, is_trend=False):
    ...
    try:
        ...
        tomito_opinion = (tri or {}).get('opinion') or generate_tomito_opinion(title_ar, title_en, year, media_type, ai_data=tri)
